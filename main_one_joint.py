...
def train(START_ITER_MOD, NUM_ITER_TO_TRAIN, model, optimizer, train_loader, epoch, total_loss, total_dist_loss, total_dist_loss_sample,
          curr_iter, losses, dist_losses, dist_loss_sample, best_loss, best_dist_loss, best_dist_loss_sample,
          best_model_dict, tot_toc):
    curr_train_ix = 1
    tot_iter = int(len(train_loader) / int(BATCH_SIZE/MAX_MEM_BATCH_SIZE))
    tot_iter_time = tot_iter
    curr_epoch_iter = 1
    done = False
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        total_dist_loss = my_losses.accumulate_pixel_dist_loss(
            total_dist_loss, output[-1], target, BATCH_SIZE)
        total_dist_loss_sample = my_losses.accumulate_pixel_dist_loss_from_sample(
            total_dist_loss_sample, output[-1], target, BATCH_SIZE)
        ...
        if minibatch_completed:
            ...
            total_loss = 0
            # append dist loss
            dist_losses.append(total_dist_loss)
            # erase pixel dist loss
            total_dist_loss = 0
            # append dist loss of sample from output
            dist_losses_sample.append(total_dist_loss_sample)
            # erase dist loss of sample from output
            total_dist_loss_sample = 0
            ...
            if losses[-1] < best_loss:
                ...
                best_model_dict = {
                    ...
                    'curr_iter': curr_iter,
                    'train_ix': curr_train_ix
                }
            # log checkpoint
            if curr_iter % LOG_INTERVAL == 0:
                print("\nSaving best model and checkpoint model")
                ...
                print("\nTraining set mean error for last " + str(LOG_INTERVAL) +
                ...
                print("Training set mean error for last " + str(LOG_INTERVAL) +
                      " iterations (average pixel loss): " + str(np.mean(dist_losses[-LOG_INTERVAL:])))
                print("Training set stddev error for last " + str(LOG_INTERVAL) +
                      " iterations (average pixel loss): " + str(np.std(dist_losses[-LOG_INTERVAL:])))
                print("This is the last pixel dist loss: " + str(dist_losses[-1]))
                print("Training set mean error for last " + str(LOG_INTERVAL) +
                      " iterations (average pixel loss of sample): " + str(np.mean(dist_losses_sample[-LOG_INTERVAL:])))
                print("Training set stddev error for last " + str(LOG_INTERVAL) +
                      " iterations (average pixel loss of sample): " + str(np.std(dist_losses_sample[-LOG_INTERVAL:])))
                print("This is the last pixel dist loss of sample: " + str(dist_losses_sample[-1]))
...
for epoch in range(NUM_EPOCHS):
    ...
    best_model_dict, tot_toc =\
        train(START_ITER_MOD, NUM_ITER_TO_TRAIN, halnet, optimizer, train_loader, epoch + 1, total_loss,
              total_dist_loss, total_dist_loss_sample, curr_iter, losses, dist_losses, dist_losses_sample,
              best_loss, best_dist_loss, best_dist_loss_sample, best_model_dict, tot_toc)
    if done:
        break
    if epoch + 1 >= START_EPOCH:
