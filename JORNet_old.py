...
class JORNet(nn.Module):
    ...
    def __init__(self, joint_ixs, use_cuda=True):
        ...
        self.innerproduct1 = nn.Linear(in_features=256*30*40, out_features=200)
        if use_cuda:
            self.innerproduct1 = cudafy(self.innerproduct1)
        self.innerproduct2 = nn.Linear(in_features=200, out_features=self.num_joints*3)
        if use_cuda:
            self.innerproduct2 = cudafy(self.innerproduct2)
    ...
    def forward(self, x):
        ...
        return out_main, outinner_joints
