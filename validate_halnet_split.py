...
def validate(valid_loader, model, optimizer, valid_vars, control_vars, verbose=True):
    ...
    for batch_idx, (data, target) in enumerate(valid_loader):
        ...
        if minibatch_completed:
            ...
            if control_vars['curr_iter'] % control_vars['log_interval'] == 0:
                tot_joint_loss_avg = trainer.print_log_info(model, optimizer, 1,
                                                            total_loss, valid_vars, control_vars, save_best=False)
                model_dict = {
    ...
    return valid_vars, control_vars, tot_joint_loss_avg
...
dataset = synthhands_handler.SynthHandsDataset(root_folder=valid_vars['root_folder'],
                                               type_='split',
