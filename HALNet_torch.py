...
class Softmax2DProbability(torch.nn.Module):
    ...
    def forward(self, x, eps=1e-9):
        ...
        softmax_ = F.softmax(x.view((orig_shape[0], orig_shape[2] * orig_shape[3])), dim=1).view(orig_shape)
        return (softmax_ + eps).log()
...
class HALNet(nn.Module):
    ...
    def __init__(self, use_cuda=True):
        ...
        self.interm_loss1_softmax = Softmax2DProbability()
        if use_cuda:
        ...
        self.interm_loss2_softmax = Softmax2DProbability()
        if use_cuda:
        ...
        self.interm_loss3_softmax = Softmax2DProbability()
        if use_cuda:
        ...
        self.softmax_final = Softmax2DProbability()
        if use_cuda:
